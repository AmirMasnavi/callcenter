...
export default function SupervisorPage({canVoid=false,canReopen=false}:{canVoid?:boolean,canReopen?:boolean}){
 ...
 {q.isLoading?<Loading/>:<div className="review-layout"><section className="queue">{!filtered.length&&<div className="empty compact">گزارشی در این بخش نیست.</div>}{filtered.map(r=><button className={selected?.id===r.id?'active':''} key={r.id} onClick={()=>choose(r)}><div className="avatar">{r.agentName.slice(0,1)}<img src={apiUrl(`/api/v1/users/${r.agentId}/avatar`)} onError={e=>e.currentTarget.style.display='none'}/></div><div><b>{r.agentName}</b><small>{r.reportLabel||'بدون عنوان'} · {faDate(r.reportDate)}</small><small>{faDateTime(r.submittedAt||r.createdAt)}</small></div><span className={'status '+r.status}>{statusLabel[r.status]}</span></button>)}</section>
