...
type Form={reportLabel:string;totalPeople:number;contactedCount:number;okCount:number;maybeCount:number;noCount:number;noAnswerCount:number;notes:string};
const empty:Form={reportLabel:'',totalPeople:0,contactedCount:0,okCount:0,maybeCount:0,noCount:0,noAnswerCount:0,notes:''};
const localKey=(date:string,id?:number)=>`agent-draft:${date}:${id||'new'}`;
const toForm=(r:Report):Form=>({reportLabel:r.reportLabel||'',totalPeople:r.totalPeople,contactedCount:r.contactedCount,okCount:r.okCount,maybeCount:r.maybeCount,noCount:r.noCount,noAnswerCount:r.noAnswerCount,notes:r.notes||''});
...
export default function AgentPage({view}:{view:'form'|'history'}){
 ...
 function update(next:Form){setForm(next);sessionStorage.setItem(localKey(date,activeId),JSON.stringify(next))}
 function select(r:Report){setDate(r.reportDate);setActiveId(r.id);setNotice('')}
 function newReport(){setActiveId(undefined);setVersion(undefined);setForm(empty);setNotice('گزارش تازه آماده است.')}
 ...
 function number(key:Exclude<keyof Form,'notes'|'reportLabel'>,value:string){const normalized=value.replace(/[۰-۹]/g,d=>String('۰۱۲۳۴۵۶۷۸۹'.indexOf(d)));update({...form,[key]:Math.max(0,Number(normalized)||0)})}
 ...
 const valid=!errors.length,locked=!!current&&(current.status==='APPROVED'||current.status==='CORRECTED_APPROVED');
 const payload=()=>({...form,reportId:activeId||null,reportDate:date,version});
