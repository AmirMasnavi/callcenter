...
import{api,apiUrl,Me}from'./lib/api';
...
export default function App(){
 ...
 return <div className="app-shell"><aside><div className="brand"><img src="/brand-logo.png" alt="لوگو" className="brand-logo"/><div><b>گزارش‌یار</b><span>علم و صنعت آریا</span></div></div><nav>{nav.map(n=><button key={n[0]} className={active===n[0]?'active':''} onClick={()=>navigate(n[0])}><i>{n[1]}</i>{n[2]}</button>)}</nav><div className="aside-user"><div className="avatar">{user.displayName.slice(0,1)}<img src={apiUrl(`/api/v1/users/${user.id}/avatar`)} onError={e=>e.currentTarget.style.display='none'}/></div><div><b>{user.displayName}</b><span>{{AGENT:'اپراتور',SUPERVISOR:'ناظر',MANAGER:'مدیر',ADMIN:'ادمین'}[user.role]}</span></div><button className="logout-btn" onClick={logout} title="خروج"><i>↪</i><span>خروج</span></button></div></aside>
