...
public interface DailyReportRepository extends JpaRepository<DailyReport,Long>{
 List<DailyReport> findByAgentIdAndVoidedAtIsNullOrderByReportDateDescCreatedAtDesc(Long agentId);

 @Query("select r from DailyReport r join fetch r.agent a where r.status=:status and a.supervisor.id=:supervisor and r.voidedAt is null order by r.submittedAt")
 List<DailyReport> pending(@Param("supervisor") Long supervisor,@Param("status") ReportStatus status);

 @Query("select r from DailyReport r join fetch r.agent a left join fetch r.reviewer where a.supervisor.id=:supervisor and r.status<>com.elmosanatearia.callcenter.report.ReportStatus.DRAFT and r.voidedAt is null order by r.reportDate desc,r.createdAt desc")
 ...
 @Query("select r from DailyReport r join fetch r.agent a where r.status=:status and r.voidedAt is null order by r.submittedAt")
