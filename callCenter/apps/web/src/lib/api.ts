...
export interface Report{id:number;agentId:number;agentName:string;reportDate:string;totalPeople:number;contactedCount:number;notContacted:number;okCount:number;maybeCount:number;noCount:number;noAnswerCount:number;notes?:string;status:Status;submittedAt?:string;reviewedAt?:string;reviewerName?:string;version:number}
function cookie(name:string){return document.cookie.split('; ').find(x=>x.startsWith(name+'='))?.split('=').slice(1).join('=');}
export async function api<T>(path:string,init:RequestInit={}):Promise<T>{
 const method=init.method?.toUpperCase()||'GET';if(method!=='GET'&&!cookie('XSRF-TOKEN'))await fetch('/api/v1/auth/csrf',{credentials:'include'});
 const headers=new Headers(init.headers);if(init.body)headers.set('Content-Type','application/json');const token=cookie('XSRF-TOKEN');if(token)headers.set('X-XSRF-TOKEN',decodeURIComponent(token));
